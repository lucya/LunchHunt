...
export class XenovaRecommender implements AIRecommender {
  ...
  private getRestaurantImageFromSearch(
    ...
    // 음식 종류별 실제 음식점 스타일 이미지 키워드
    const restaurantImageKeywords: { [key: string]: string[] } = {
      한식: [
    ...
    const keywords = restaurantImageKeywords[foodType] || ["restaurant-food"];
    const selectedKeyword = keywords[(restaurantId - 1) % keywords.length];
